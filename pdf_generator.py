...
class PDFGenerator:
    ...
    def _download_fonts_if_missing(self, font_dir):
        """Download Noto Sans fonts from Google Fonts if missing"""
        fonts_to_download = {
        ...
        for font_file, url in fonts_to_download.items():
            font_path = os.path.join(font_dir, font_file)
            if not os.path.exists(font_path):
                print(f"[PDFGenerator] Downloading missing font {font_file}...")
                try:
                    urllib.request.urlretrieve(url, font_path)
                    print(f"[PDFGenerator] Downloaded {font_file}")
                except Exception as e:
                    print(f"[PDFGenerator] Error downloading {font_file}: {str(e)}")
    ...
    def _register_fonts(self):
        ...
        # Primary font directory we will download to if needed
        primary_font_dir = os.path.join(base_dir, "fonts")
        self._download_fonts_if_missing(primary_font_dir)
